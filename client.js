...
function switchTab(tabName, event) {
    ...
    if (!isMobile) return; // На ПК табы не переключаются
    ...
    if (tabName === 'chats') {
        ...
        // На мобилке "Контакты" — это поиск и добавление друзей (в chat-menu уже есть поиск)
        if (chatMenu) chatMenu.style.display = 'block';
...
function processLogin() {
    ...
    socket.emit('login', { login, password: pass }, (res) => {
        ...
        myId = login; // внутренний ключ для комнат/чатов — логин пользователя
        myName = res.name;
        myGhost = res.ghost;
        ...
        // Сразу прячем ссылки "Забыли пароль?" / "Нет аккаунта?", чтобы они не "висели"
        // на экране, пока карточка улетает вниз.
        const outerLinks = document.getElementById('outerLinksHolder');
        if (outerLinks) outerLinks.style.display = 'none';

        // Когда карточка уже почти улетела — меняем экран входа на экран загрузки
        // мессенджера, вместо того чтобы показывать пустой экран входа.
        setTimeout(() => {
...
socket.on('newMessage', ({ chatId, msg }) => {
    ...
    if (currentChatFriendId && chatId === pairId(myId, currentChatFriendId)) {
        ...
        renderMessages();
        // Если это сообщение от друга и чат открыт — сразу подтягиваем актуальные статусы
